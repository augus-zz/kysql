...
SQLEditorHighlighter::SQLEditorHighlighter ( QTextDocument *parent ) :
    ...
    m_colors[SQLEditor::Normal]     = QColor(0, 0, 0);
    m_colors[SQLEditor::Comment]    = QColor(128, 128, 128);

    m_colors[SQLEditor::Operator]   = QColor(0, 0, 0);

    m_colors[SQLEditor::Number]     = QColor(64, 0, 0);
    m_colors[SQLEditor::String]     = QColor(0, 160, 0);

    m_colors[SQLEditor::Keyword]    = QColor(160, 0, 96);
    m_colors[SQLEditor::Function]   = QColor(96, 0, 160);
    m_colors[SQLEditor::Interval]   = QColor(160, 0, 160);

    m_colors[SQLEditor::Type]       = QColor(0, 96, 96);
    m_colors[SQLEditor::Identifier] = QColor(0, 32, 192);

    m_colors[SQLEditor::Marker]     = QColor(255, 255, 0);
}

void
SQLEditorHighlighter::setHighlightSQLEditor ( SQLEditor *highlight_sql_editor ) {
    Highlight_SQLEditor = highlight_sql_editor;
}

void
SQLEditorHighlighter::setColor ( SQLEditor::ColorComponent component,
                                     const QColor &color ) {
...
void
SQLEditorHighlighter::highlightBlock ( const QString &text ) {
    // parsing state
    enum {
        Start = 0,
        Number = 1,
    ...
    while (i <= text.length()) {
        ...
        case Start:
            ...
            if (ch.isSpace()) {
                ...
                i = text.length();
                setFormat(start, text.length(), m_colors[SQLEditor::Comment]);
            } else {
                if (not QString("(){}[]").contains(ch))
                    setFormat(start, 1, m_colors[SQLEditor::Operator]);
        ...
        case Number:
            if (ch.isSpace() or
                (not (ch.isDigit() or (ch == '.') or
                      (ch == '+') or (ch == '-') or
                      (ch == 'E') or (ch == 'e')))) {
                setFormat(start, i - start, m_colors[SQLEditor::Number]);
                state = Start;
            } else {
                ++i;
            }
        ...
        case Identifier:
            if (ch.isSpace() or
                ...
                if (token.startsWith('.') or (ch == '.'))
                    setFormat(start, i - start, m_colors[SQLEditor::Identifier]);
                else if (Highlight_SQLEditor->isKeyword(token.toUpper()))
                    setFormat(start, i - start, m_colors[SQLEditor::Keyword]);
                else if (Highlight_SQLEditor->isFunction(token.toUpper()))
                    setFormat(start, i - start, m_colors[SQLEditor::Function]);
                else if (Highlight_SQLEditor->isType(token.toUpper()))
                    setFormat(start, i - start, m_colors[SQLEditor::Type]);
                else if (Highlight_SQLEditor->isInterval(token.toUpper()))
                    setFormat(start, i - start, m_colors[SQLEditor::Interval]);
                else
                    setFormat(start, i - start, m_colors[SQLEditor::Identifier]);
                state = Start;
            } else {
        ...
        case String:
            if ((ch == '\\') and
                ...
                if ((not (prev == '\\')) or ((prev_prev == '\\') and (prev == '\\'))) {
                    ++i;
                    setFormat(start, i - start, m_colors[SQLEditor::String]);
                    state = Start;
                }
                else {
        ...
        case Comment:
            if ((ch == '*') and (next_ch == '/')) {
                ++i; // "*/" part of comment ...
                ++i; // ... incorporate
                setFormat(start, i - start, m_colors[SQLEditor::Comment]);
                state = Start;
            } else {
                ++i;
            }
    ...
    if (!m_markString.isEmpty()) {
        ...
        markerFormat.setBackground(m_colors[SQLEditor::Marker]);
        markerFormat.setForeground(m_colors[SQLEditor::Normal]);
        for (;;) {
    ...
    if (!bracketPositions.isEmpty()) {
        SQLBlockData *blockData = reinterpret_cast<SQLBlockData*>(currentBlock().userData());
        if (!blockData) {
            blockData = new SQLBlockData;
...
void
SQLEditorHighlighter::mark ( const QString &str,
...
SQLEditorSidebar::SQLEditorSidebar ( SQLEditor *editor ) : QWidget ( editor ),
...
void
SQLEditorSidebar::mousePressEvent ( QMouseEvent *event ) {
    if (foldIndicatorWidth > 0) {
        ...
        if (lineNo >= 0) {
            SQLEditor *editor = qobject_cast<SQLEditor*>(parent());
            if (editor)
...
void
SQLEditorSidebar::paintEvent ( QPaintEvent *event ) {
    QPainter p(this);
...
SQLEditorDocLayout::SQLEditorDocLayout ( QTextDocument *doc ) : QPlainTextDocumentLayout ( doc ) {
}

void
SQLEditorDocLayout::forceUpdate ( ) {
    emit documentSizeChanged(documentSize());
}

SQLEditor::SQLEditor ( QWidget *parent ) : QPlainTextEdit( parent ) {
    Editor_Layout = new SQLEditorDocLayout(document());
    Editor_Highlighter = new SQLEditorHighlighter(document());
    ...
    Editor_Highlighter->setHighlightSQLEditor(this);
    Editor_Sidebar = new SQLEditorSidebar(this);
    ...
    SQL_Keywords.clear();
    SQL_Keywords << "ACCESS" << "ADD" << "ALL" << "ALTER" << "ANALYZE";
    SQL_Keywords << "AND" << "AS" << "ASC" << "AUTO_INCREMENT" << "BDB";
    SQL_Keywords << "BERKELEYDB" << "BETWEEN" << "BOTH" << "BY" << "CASCADE";
    SQL_Keywords << "CASE" << "CHANGE" << "CHARSET" << "COLUMN" << "COLUMNS";
    SQL_Keywords << "CONSTRAINT" << "CREATE" << "CROSS" << "CURRENT_DATE" << "CURRENT_TIME";
    SQL_Keywords << "CURRENT_TIMESTAMP" << "DATABASE" << "DATABASES" << "DAY_HOUR" << "DAY_MINUTE";
    SQL_Keywords << "DAY_SECOND" << "DEC" << "DEFAULT" << "DELAYED" << "DELETE";
    SQL_Keywords << "DESC" << "DESCRIBE" << "DISTINCT" << "DISTINCTROW" << "DROP";
    SQL_Keywords << "ELSE" << "ENCLOSED" << "ESCAPED" << "EXISTS" << "EXPLAIN";
    SQL_Keywords << "FIELDS" << "FOR" << "FOREIGN" << "FROM" << "FULLTEXT";
    SQL_Keywords << "FUNCTION" << "GRANT" << "GROUP" << "HAVING" << "HIGH_PRIORITY";
    SQL_Keywords << "IF" << "IGNORE" << "IN" << "INDEX" << "INFILE";
    SQL_Keywords << "INNER" << "INNODB" << "INSERT" << "INTERVAL" << "INTO" << "IS";
    SQL_Keywords << "JOIN" << "KEY" << "KEYS" << "KILL" << "LEADING";
    SQL_Keywords << "LEFT" << "LIKE" << "LIMIT" << "LINES" << "LOAD";
    SQL_Keywords << "LOCK" << "LOW_PRIORITY" << "MASTER_SERVER_ID" << "MATCH" << "MRG_MYISAM";
    SQL_Keywords << "NATIONAL" << "NATURAL" << "NOT" << "NULL" << "NUMERIC";
    SQL_Keywords << "ON" << "OPTIMIZE" << "OPTION" << "OPTIONALLY" << "OR";
    SQL_Keywords << "ORDER" << "OUTER" << "OUTFILE" << "PARTIAL" << "PRECISION";
    SQL_Keywords << "PRIMARY" << "PRIVILEGES" << "PROCEDURE" << "PURGE" << "READ";
    SQL_Keywords << "REFERENCES" << "REGEXP" << "RENAME" << "REPLACE" << "REQUIRE";
    SQL_Keywords << "RESTRICT" << "RETURNS" << "REVOKE" << "RIGHT" << "RLIKE";
    SQL_Keywords << "SELECT" << "SET" << "SHOW" << "SONAME" << "SQL_BIG_RESULT";
    SQL_Keywords << "SQL_CALC_FOUND_ROWS" << "SQL_SMALL_RESULT" << "SSL" << "STARTING" << "STRAIGHT_JOIN";
    SQL_Keywords << "STRIPED" << "TABLE" << "TABLES" << "TERMINATED" << "THEN";
    SQL_Keywords << "TO" << "TRAILING" << "TRUNCATE" << "TYPE" << "UNION";
    SQL_Keywords << "UNIQUE" << "UNLOCK" << "UNSIGNED" << "UPDATE" << "USAGE";
    SQL_Keywords << "USE" << "USER_RESOURCES" << "USING" << "VALUES" << "VARYING";
    SQL_Keywords << "WHEN" << "WHERE" << "WHILE" << "WITH" << "WRITE";
    SQL_Keywords << "XOR" << "YEAR_MONTH" << "ZEROFILL";
    ...
    SQL_Functions.clear();
    SQL_Functions << functions << math_functions << date_time_functions;
    SQL_Functions << cast_functions << misc_functions << aggregate_functions;
    ...
    SQL_Types.clear();
    SQL_Types << strings_types << numeric_types << date_time_types;
    ...
    SQL_Intervals.clear();
    SQL_Intervals << date_time_intervals;

    All_SQL_Keywords.clear();
    All_SQL_Keywords << SQL_Keywords << SQL_Functions << SQL_Types << SQL_Intervals;
    ...
    // this->setColor(SQLEditor::Background,    QColor(255, 255, 255));
    // this->setColor(SQLEditor::Normal,        QColor(0, 0, 0));
    // this->setColor(SQLEditor::Comment,       QColor(128, 128, 128));
    // this->setColor(SQLEditor::Number,        QColor(192, 0, 0));
    // this->setColor(SQLEditor::String,        QColor(0, 128, 0));
    // this->setColor(SQLEditor::Operator,      QColor(0, 0, 0));
    // this->setColor(SQLEditor::Identifier,    QColor(128, 0, 128));
    // this->setColor(SQLEditor::Keyword,       QColor(0, 160, 160));
    // this->setColor(SQLEditor::Function,      QColor(0, 128, 192));
    // this->setColor(SQLEditor::Type,          QColor(0, 128, 192));
    // this->setColor(SQLEditor::Interval,      QColor(0, 128, 192));
    // this->setColor(SQLEditor::Cursor,        QColor(255, 255, 192));
    // this->setColor(SQLEditor::Marker,        QColor(255, 255, 0));
    // this->setColor(SQLEditor::BracketMatch,  QColor(128, 255, 128));
    // this->setColor(SQLEditor::BracketError,  QColor(255, 128, 128));
    // this->setColor(SQLEditor::FoldIndicator, Qt::lightGray);
    ...
    QString bracket_characters = QString("[") + QString(SQL_Bracket_List) + QString("]");
...
QString
SQLEditor::Replace_Paragraph_Separator ( QString Initial_Text ) {
    // 0x2029 paragraph separator
    return Initial_Text.replace(QChar(0x2029), QChar('\n'));
}

void
SQLEditor::Set_PlainText ( QString Set_Plain_Text ) {
...
QString
SQLEditor::Selected_Text ( ) {
    QString sel_text = QPlainTextEdit::textCursor().selectedText();
    return Replace_Paragraph_Separator(sel_text);
}

QStringList
SQLEditor::sqlKeywords ( ) const {
    return SQL_Keywords;
}

QStringList
SQLEditor::sqlFunctions ( ) const {
    return SQL_Functions;
}

QStringList
SQLEditor::sqlTypes ( ) const {
    return SQL_Types;
}

QStringList
SQLEditor::sqlIntervals ( ) const {
    return SQL_Intervals;
}

bool
SQLEditor::isKeyword ( QString potential_keyword ) {
    return SQL_Keywords.contains(potential_keyword);
}

bool
SQLEditor::isFunction ( QString potential_function ) {
    return SQL_Functions.contains(potential_function);
}


bool
SQLEditor::isType ( QString potential_type ) {
    return SQL_Types.contains(potential_type);
}


bool
SQLEditor::isInterval ( QString potential_interval ) {
    return SQL_Intervals.contains(potential_interval);
}


QString
SQLEditor::Compute_Bracket_Text ( QString Source_Text ) {
    QString bracket_list = SQL_Bracket_List;
    QString bracket_text = QString(" ").repeated(Source_Text.length());
    if (SQL_Bracket_RegEx.isValid()) {
        QRegularExpressionMatchIterator regex_iterator =
                                          SQL_Bracket_RegEx.globalMatch(Source_Text);
...
int
SQLEditor::Bracket_Match_Position ( int Current_Position ) {
...
int
SQLEditor::Compute_Current_Paren_Indent ( QString Current_Text,
...
QTextCursor
SQLEditor::Select_Selected_Text_Lines ( ) {
...
void
SQLEditor::Indent_Selected_Text_Lines ( int Indent_Tab_Modulus ) {
...
void
SQLEditor::insertFromMimeData ( const QMimeData* source ) {
    if (source->hasText()) {
...
SQLEditor::onCursorPositionChanged ( ) {
    ...
    if (AutoUppercaseKeywordsEnabled) {
        if (not (cursor_line == Previous_Cursor_Line)) {
            // int previous_line_ch_idx = SQLEditor->positionFromLineIndex(Previous_Cursor_Line, 0);
            // int previous_line_length = SQLEditor->lineLength(Previous_Cursor_Line);
...
void
SQLEditor::Uppercase_SQL_Keywords ( int start_uppercase_position,
        ...
        while  ((sql_text_idx >= 0) and
            ...
            if (word_idx >= 0) {
                ...
                if (word.length() > 0) {
                    ...
                    if (All_SQL_Keywords.contains(word.toUpper()) and
...
QString
SQLEditor::Initial_SQL_Keyword ( QString SQL_Statement ) {
...
SQLEditor::Simple_Format_SQL ( ) {
    ...
    QString significant_punctuation = QString(SQL_Bracket_List) + ",;";
...
void
SQLEditor::setColor ( ColorComponent component,
                         const QColor &color ) {
...
void
SQLEditor::setShowLineNumbersEnabled ( bool enable ) {
    ShowLineNumbersEnabled = enable;
    updateSidebar();
}

bool
SQLEditor::isShowLineNumbersEnabled ( ) const {
    return ShowLineNumbersEnabled;
}

void
SQLEditor::setTextWrapEnabled ( bool enable ) {
    TextWrapEnabled = enable;
    setLineWrapMode(enable ? WidgetWidth : NoWrap);
}

bool
SQLEditor::isTextWrapEnabled ( ) const {
    return TextWrapEnabled;
}

void
SQLEditor::setBracketsMatchingEnabled ( bool enable ) {
    BracketsMatchingEnabled = enable;
    updateCursor();
}

bool
SQLEditor::isBracketsMatchingEnabled ( ) const {
    return BracketsMatchingEnabled;
}

void
SQLEditor::setAutoUppercaseKeywordsEnabled ( bool enable ) {
   AutoUppercaseKeywordsEnabled = enable;
}

bool
SQLEditor::isAutoUppercaseKeywordsEnabled ( ) const {
    return AutoUppercaseKeywordsEnabled;
}

void
SQLEditor::setAutoIndentEnabled ( bool enable ) {
    AutoIndentEnabled = enable;
}

bool
SQLEditor::isAutoIndentEnabled ( ) const {
    return AutoIndentEnabled;
}

void
SQLEditor::Set_Tab_Modulus ( int New_Tab_Modulus ) {
    Tab_Modulus = New_Tab_Modulus;
}

void
SQLEditor::setCodeFoldingEnabled ( bool enable ) {
    CodeFoldingEnabled = enable;
    updateSidebar();
}

bool
SQLEditor::isCodeFoldingEnabled ( ) const {
    return CodeFoldingEnabled;
}

bool
SQLEditor::isFoldable ( int line ) {
...
bool
SQLEditor::isFolded ( int line ) const {
...
SQLEditor::fold ( int line ) {
    ...
    SQLEditorDocLayout *layout = reinterpret_cast<SQLEditorDocLayout*>(document()->documentLayout());
    layout->forceUpdate();
}

void
SQLEditor::unfold ( int line ) {
    QTextBlock startBlock = document()->findBlockByNumber(line - 1);
    int endPos = findClosingConstruct(startBlock);
...
int
SQLEditor::findClosingConstruct ( const QTextBlock &block ) {
    if (!block.isValid())
        return -1;
    SQLBlockData *blockData = reinterpret_cast<SQLBlockData*>(block.userData());
    if (!blockData)
        return -1;
...
void
SQLEditor::resizeEvent ( QResizeEvent *event ) {
    QPlainTextEdit::resizeEvent(event);
    updateSidebar();
}

void
SQLEditor::wheelEvent ( QWheelEvent *event ) {
...
void
SQLEditor::updateSidebar ( const QRect &rect,
                              int d ) {
    Q_UNUSED(rect)
    if (d != 0)
        updateSidebar();
...
void
SQLEditor::mark ( const QString &str,
...
// Auto complete ...
void
SQLEditor::initializeAutoComplete ( ) {
    if (AutoCompleteKeywordsEnabled or AutoCompleteIdentifiersEnabled) {
        QStringList word_list;

        if (AutoCompleteKeywordsEnabled) word_list << All_SQL_Keywords;
...
void
SQLEditor::finalizeAutoComplete ( ) {
    if (Completer and Completer->popup()->isVisible()) Completer->popup()->hide();
}

void
SQLEditor::setAutoCompleteKeywordsEnabled ( bool enable ) {
    AutoCompleteKeywordsEnabled = enable;
    initializeAutoComplete();
}

bool
SQLEditor::isAutoCompleteKeywordsEnabled ( ) const {
    return AutoCompleteKeywordsEnabled;
}

void
SQLEditor::setAutoCompleteIdentifierList ( QStringList identifier_list ) {
    Auto_Complete_Identifier_List = identifier_list;
    initializeAutoComplete();
}

void
SQLEditor::setAutoCompleteContextIdentifierList ( QHash <QString, QStringList> context_identifier_list ) {
    Auto_Complete_Context_Identifier_List = context_identifier_list;
    initializeAutoComplete();
}

void
SQLEditor::setAutoCompleteIdentifiersEnabled ( bool enable ) {
    AutoCompleteIdentifiersEnabled = enable;
    initializeAutoComplete();
}

bool
SQLEditor::isAutoCompleteIdentifiersEnabled ( ) const {
    return AutoCompleteIdentifiersEnabled;
}

void
SQLEditor::setCompleter ( QCompleter *completer ) {
...
void
SQLEditor::insertCompletion ( const QString& completion,
                                 bool replace_entire ) {
...
QString
SQLEditor::textUnderCursor ( ) const {
    QTextCursor txt_cur = textCursor();
    txt_cur.select(QTextCursor::WordUnderCursor);
...
void
SQLEditor::focusInEvent ( QFocusEvent *event ) {
